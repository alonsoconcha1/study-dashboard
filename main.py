...
def cargar_parametros():
    ...
    df_pacientes = df_pacientes.head(N_PACIENTES)
    df_pabellones = df_pabellones.head(N_PABELLONES)

    pabellones_seleccionados = df_pabellones["id_pabellon"].tolist()
    df_costos = df_costos[df_costos["id_pabellon"].isin(pabellones_seleccionados)]
    df_operativos = df_operativos[df_operativos["id_pabellon"].isin(pabellones_seleccionados)]

    df_bloques = df_bloques[df_bloques["dia"] <= N_DIAS]

    bloques_seleccionados = df_bloques["bloque"].tolist()
    df_operativos = df_operativos[df_operativos["bloque"].isin(bloques_seleccionados)]

    df_costos = df_costos[df_costos["dia"] <= N_DIAS]

    print("Instancia reducida: 500 pacientes, 9 pabellones, 22 días")
    ...
    for j in J:
        for d in D:
            bloques_dia = T_d[d]
            normales = [t for t in bloques_dia if n.get((j, t), 0) == 1]

            if len(normales) > 0:
                ultimo_normal = max(normales)
                extras = [
                    t for t in bloques_dia
                    if ultimo_normal < t <= ultimo_normal + 2
                ]
    ...
    datos = {
        ...
        "parametros": {
            ...
            "lambda_factor": lambda_factor, "n": n, "a": a, "phi": phi, "pi": pi
        }
    }
...
def resolver_modelo_mejorada(datos):
    ...
    capacidad_regular = gp.quicksum(
        n[j, t] for j in J for t in T
    )

    modelo.addConstr(
        uso_regular >= mu * capacidad_regular,
        name="utilizacion_minima_operativa"
    )
    ...
    if modelo.status == GRB.OPTIMAL or modelo.status == GRB.TIME_LIMIT:
        if modelo.SolCount > 0:
            ...
            open_blocks = sum(n[j, t] for j in J for t in T)
            ...
            cleaning_blocks_regular = sum(
                L[j, t].X
                for (j, t) in L_index
                if n.get((j, t), 0) == 1
            )

            occupied_blocks_regular = sum(
                O[key].X
                for key in O_index
                if n.get((key[1], key[2]), 0) == 1
            )

            extra_blocks_used = sum(
                H[j, d].X
                for j in J
                for d in D
            )
            ...
            print(f"Bloques normales disponibles: {open_blocks:.0f}")
            print(f"Bloques de cirugia: {surgery_blocks:.0f}")
            print(f"Bloques de limpieza en horario regular: {cleaning_blocks_regular:.0f}")
            print(f"Bloques extra utilizados: {extra_blocks_used:.0f}")
            print(f"Utilizacion quirurgica neta: {surgery_blocks / open_blocks:.2%}")
            print(f"Utilizacion operativa regular: {(occupied_blocks_regular + cleaning_blocks_regular) / open_blocks:.2%}")
